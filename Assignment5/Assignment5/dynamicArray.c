...
void adjustHeap(DynamicArray* heap, int last, int position, compareFunction compare)
{
	...
	if (rightIdx < last)
	{
		...
		if (compare(dyGet(heap, position), dyGet(heap, minIdx)) == 1)
		{
	...
	{
		if (compare(dyGet(heap, position), dyGet(heap, leftIdx)) == 1)
		{
...
{
    // FIXME: implement
	// function based on the buildHeap function shown in worksheet 34
	int max = dySize(heap);
...
{
    // FIXME: implement
	// function based on the heapsort function shown in worksheet 34
